...
from sems_vision.yolov3_detecting_processor import YoloV3DetectingProcessor


def main():
    frame_src = camera_server_frame_src('10.22.244.185', 3002, 4)

    detector = YoloV3DetectingProcessor(0.5, 0.5)
    detecting_processor = detector.process(frame_src)

    executor = imshow_pipeline_executor(detector)
